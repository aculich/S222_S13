...
\frametitle{Features for Logistic Regression}	
      ...
      \item determine whether each video won more than it lost, or vice versa.\\
               videos that won more than lost are ``winners''\\
               videos that lost more than won are ``losers''\vspace{2mm}
...
=======
             percentage of loser comments that contain $w$.\vspace{2mm}
      \item calculate $logOdds = \mathnormal{\log}(\frac{\mathbb{P}(winner | comments)}{\mathbb{P}(loser | comments)}) $ for each video id: \vspace{3mm}
>>>>>>> updating Bayes section
...
$\mathnormal{n}$: number of comments in the video \\
$\mathnormal{m_w}$: number of comments containing $\mathnormal{w}.$  \vspace{3mm}
%\end{equation}
      \end{enumerate}
    \end{beamerboxesrounded}
